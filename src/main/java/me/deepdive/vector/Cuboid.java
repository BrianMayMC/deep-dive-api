 ...
 * It automatically does all the vector calculations for you, a LOT is done for you. Check all the pre-created methods for information on what's possible
 * Chances are, anything you might need, is already created.
 */
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    @Getter protected final int x2, y2, z2;
    ...
    /**
     * Get the locations in the Cuboid
     *
     * @return The locations in the cuboid
     */
    public List<Location> getLocations(){
        Iterator<Location> locI = this.locationIterator();
    ...
    public Cuboid expand(int percentage){
        double increaseAmount = 1 + (percentage/100f);
        int newMinX = (int) Math.floor(getLowerX() * increaseAmount);
        int newMinZ = (int) Math.floor(getLowerZ() * increaseAmount);
        int newMaxX = (int) Math.ceil(getUpperX() * increaseAmount);
        int newMaxZ = (int) Math.ceil(getUpperZ() * increaseAmount);

        return new Cuboid(getWorld(), newMinX, getLowerY(), newMinZ, newMaxX, getUpperY(), newMaxZ);
    }
    ...
    public boolean contains(Cuboid cuboid){
        boolean cond1 = cuboid.getUpperX() >= getLowerX();
        boolean cond2 = cuboid.getLowerX() <= getUpperX();
        boolean cond3 = cuboid.getUpperZ() >= getLowerZ();
        boolean cond4 = cuboid.getLowerZ() <= getUpperZ();
    ...
    public boolean overlaps(Cuboid cuboid){
        BoundingBox box = new BoundingBox(getLowerX(), getLowerY(), getLowerZ(), getUpperX(), getUpperY(), getUpperZ());
        BoundingBox other = new BoundingBox(cuboid.getLowerX(), cuboid.getLowerY(), cuboid.getLowerZ(), cuboid.getUpperX(), cuboid.getUpperY(), cuboid.getUpperZ());
        return box.overlaps(other);
    }

    public boolean isNear(Location location, int blocks){
        Cuboid expanded = outset(CuboidDirection.Horizontal, blocks);
...
//	}
    ...
    public class CuboidLocationIterator implements Iterator<Location> {
        ...
        public CuboidLocationIterator(World w, int x1, int y1, int z1, int x2, int y2, int z2) {
            this.w = w;
            this.baseX = x1;
            this.baseY = y1;
            this.baseZ = z1;
        ...
        public Location next() {
            Location b = new Location(w,this.baseX + this.x, this.baseY + this.y, this.baseZ + this.z);
            if (++x >= this.sizeX) {
                this.x = 0;
                if (++this.y >= this.sizeY) {
                    this.y = 0;
